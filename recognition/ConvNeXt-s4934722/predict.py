...
def predict_image(model, image_path, save_cam=False, cam_extractor=None):
    ...
    if save_cam and cam_extractor is not None:
        with torch.set_grad_enabled(True):
            ...
            # Save result
            # Determine base name (remove any existing _cam suffix)
            base_name, ext = os.path.splitext(image_path)
            if base_name.endswith("_cam"):
                base_name = base_name[:-4]  # remove "_cam"
            cam_path = base_name + "_cam.jpg"

            # Overwrite existing CAM
...
def predict_folder(model, folder_path, save_cam=False, cam_extractor=None):
    """Run inference on all images in a folder."""
    print(f"\nRunning inference on folder: {folder_path}\n")
    for fname in os.listdir(folder_path):
        if fname.lower().endswith((".png", ".jpg", ".jpeg", ".bmp")):
            fpath = os.path.join(folder_path, fname)
            label, conf = predict_image(model, fpath, save_cam=save_cam, cam_extractor=cam_extractor)
...
def main():
    ...
    parser.add_argument("--cam", action="store_true", help="Generate and save GradCAMs for predictions")
    args = parser.parse_args()

    model = load_model(args.model)

    # Initialize GradCAM only if requested
    ...
    if os.path.isfile(args.path):
        ...
        predict_folder(model, args.path, save_cam=args.cam, cam_extractor=cam_extractor)
